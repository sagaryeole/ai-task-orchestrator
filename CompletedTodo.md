...
- [x] `lint_config()` warn when `require_manual_confirmation: false` + `auto_commit: true` are combined with empty/missing `verify_commands` — that combo auto-accepts and commits any task where the agent exits 0 and touches a file, with zero correctness check. Caught while reviewing a real user config that had exactly this combination.
...
- [x] Replace the current `nargs="?"` positional-command trick (`init` / `validate` as optional positional args) with real `argparse` subparsers. This gives clean `--help` output per subcommand (`task-orchestrator init --help`, `task-orchestrator run --help`) instead of a single flat help screen.
...
- [x] Remove the manual `if "--version" in argv` check and use `parser.add_argument("--version", action="version", ...)` instead.
...
- [x] Serve static assets from `dashboard/static/` instead of inlining all CSS/JS in a Python string:
...
- [x] Add a `threading.Lock` around all mutations of `_dashboard_state`. The dashboard is mutated by the orchestrator main loop and read by `DashboardHandler` in a daemon thread; today this is "safe" only because the GIL serializes dict ops, which is an accident, not a guarantee.
...
- [x] Add a `## [Unreleased]` section at the top of `CHANGELOG.md` so contributors know where to put entry bullets.
- [x] Add `__main__.py` coverage to `tests/` — a smoke test that `python -m task_orchestrator --help` exits 0 and prints the banner/help.
